...
# convert to pyridimine for plotting
...
plotD <- ggplot(data = gene_oe[gene_oe$consequence == "synonymous",], aes(y = observed, x = expected)) +
  ...
  paper_theme +
  theme(plot.margin = unit(c(0.75, 0.25, 0.25, 0.5), "cm")) +
...
plotE <- ggplot(data = gene_oe[gene_oe$consequence == "missense" ,], aes(y = observed, x = expected)) +
  ...
  labs(x = "Expected", y = "Observed") + 
  paper_theme +
  theme(plot.margin = unit(c(0.75, 0.375, 0.25, 0.375), "cm")) +
...
plotF <- ggplot(data = gene_oe[gene_oe$consequence == "stop_gain",], aes(y = observed, x = expected)) +
  ...
  labs(x = "Expected", y = "Observed") + 
  paper_theme +
  theme(plot.margin = unit(c(0.75, 0.5, 0.25, 0.25), "cm")) +
...
ggarrange(plotA, 
          ...
          nrow = 3, ncol = 1, heights = c(0.65, 0.85, 0.7), labels = c("a", "", ""), font.label = list(size = 10))

ggsave("figures/Figure1.jpeg", width = 180, height = 170, dpi = 600, units = c("mm"))
