...
def gnomad_annotate():
	...
	for row in csv.DictReader(
		...
		if row["filters"] == "PASS":
			dict[(row["ref"], row["position"], row["alt"])] = (row["max_observed_heteroplasmy"], row["AF_hom"], row["AF_het"])
	return dict
...
def annotate(input_file: str):
	...
	for row in csv.DictReader(open(input_file), delimiter='\t'):
		...
		f.write(
			...
			str(max_hl) + '\t' + str(gnomad_af_hom) + '\t' + str(gnomad_af_het) + '\t' +
			str(in_phylo) + '\t' +
			phylop[int(row["POS"])] + '\t' +
